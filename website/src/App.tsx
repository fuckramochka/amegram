...
const REPO = 'https://github.com/fuckramochka/amegram';
...
const faq = [
  ['Что такое Amegram?', 'Amegram — независимый Android-клиент на основе Telegram: двойное хранилище, плагины MioHook и exteraGram, глубокая кастомизация, собственный музыкальный плеер и встроенный ИИ. А ещё подпапки, мультичат, бейджики и много других возможностей. Это не официальный продукт Telegram.'],
  ...
  ['Как установить Amegram?', 'Открой GitHub Releases, выбери актуальный релиз и скачай APK из раздела Assets. На Android разреши установку из выбранного источника, установи приложение и войди в свой аккаунт Telegram. Загружай файлы только из официального репозитория проекта.'],
  ...
  ['Какие плагины поддерживаются?', 'Amegram поддерживает плагины систем MioHook и exteraGram. Совместимость конкретного расширения зависит от версии плагина и клиента — поддержка системы не означает, что любой плагин будет работать без изменений. Проверяй источник, разрешения и документацию расширения.'],
  ...
  ['Будут ли ещё новые фичи?', 'Да! Amegram продолжит получать новые возможности — этот каталог не финальная точка. Следи за релизами и предлагай идеи в GitHub. Конкретные функции и сроки будут появляться по мере готовности, без обещаний неподтверждённых дат.'],
];
...
function App() {
  ...
  useEffect(() => { const stored = localStorage.getItem('amegram-demo-theme') ?? localStorage.getItem('miogram-demo-theme'); if (stored) { const n = Number(stored); if (n >= 0 && n < 3) setTheme(n); } }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    let live = true;
    fetch('https://api.github.com/repos/fuckramochka/amegram/releases/latest')
      .then(r => r.json())
      .then(data => {
        if (!live || !data) return;
  ...
  return (
    <>
      <div className="announcement"><span>✧</span> маленький апгрейд твоего цифрового мира <span>♡</span> <a href={REPO + '/releases'} target="_blank" rel="noreferrer">встречай Amegram {release?.tag || ''} <span>↗</span></a></div>
      <header className="header container">
        <a className="brand" href="#" aria-label="Amegram — главная"><WingHeart /><span>ame<span>gram</span><i>✦</i></span></a>
        <nav className={menu ? 'nav open' : 'nav'} aria-label="Основная навигация">
          <a href="#features" onClick={() => setMenu(false)}>Возможности</a><a href="#badges" onClick={() => setMenu(false)}>Бейджики</a><a href="#themes" onClick={() => setMenu(false)}>Темы <span className="tiny-heart">♡</span></a><a href="#faq" onClick={() => setMenu(false)}>FAQ</a><a href={REPO} target="_blank" rel="noreferrer">GitHub <span className="external-arrow">↗</span></a>
        </nav>
      ...
      <main>
        <section className="hero container">
          <div className="hero-copy">
            ...
            <p className="hero-description">Знакомься, <strong>Amegram</strong> — милый снаружи,<br className="desktop-break" /> мощный внутри. Твой стиль, музыка, плагины и ИИ.<br className="desktop-break" /> Целый мир возможностей. И это только начало. <span>୨୧</span></p>
          ...
          <div className="hero-visual">
            ...
            <div className="app-window" style={{ '--chat-bg': themes[theme].bg, '--chat-bubble': themes[theme].bubble } as React.CSSProperties}>
              <div className="window-bar"><span><WingHeart /> amegram.exe</span><div className="window-controls" aria-hidden="true"><span>−</span><span>□</span><span>×</span></div></div>
              <div className="app-toolbar"><span><Icon name="menu" size={15} /> Amegram <span className="app-version">♡</span></span><span className="demo-tag">live demo <span className="status-dot" /></span></div>
              ...
              <div className="app-status"><span><span className="status-dot" /> всё хорошо. ты в Amegram.</span><span>♡ connected</span></div>
            </div>
